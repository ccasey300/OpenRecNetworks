...
def assignmentTopo():
    ...
    # QoS for H1 to H3, TCP 40, 30 Mbps
    os.system(
    ...
    # QoS for H1 to H2, TCP 60, 150 Mbps
    os.system(
    'sudo ovs-vsctl set port s1-eth2 qos=@newqos -- '
    '--id=@newqos create qos type=linux-htb queues=0=@q0 -- '
    '--id=@q0 create queue other-config:min-rate=20000000 other-config:max-rate=150000000'
    )

    # =>fill in here command to add two queues to the correct port of the correct switch: you decide number of queues and rates
    # QoS for H2 to H4. 200 Mbps
    os.system(
    'sudo ovs-vsctl set port s2-eth2 qos=@newqos -- '
    '--id=@newqos create qos type=linux-htb queues=0=@q0,1=@q1 -- '
    '--id=@q0 create queue other-config:min-rate=50000000 other-config:max-rate=200000000 -- '
    '--id=@q1 create queue other-config:min-rate=100000000 other-config:max-rate=250000000'
    )
